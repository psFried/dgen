use failure::Error;
use generator::{DataGenRng, DynGenerator, Generator, GeneratorArg};
    ...
    > ClosureArgumentInner<R, T>
{
    ...
    fn update_value_if_needed(&mut self, rng: &mut DataGenRng) -> Result<(), Error> {
        if self.resetter.is_value_reset() {
            let ClosureArgumentInner {
                ref mut wrapped_gen,
                ref mut memoized_value,
                ref resetter,
                ..
            } = *self;
    ...
    fn write_bytes(
        ...
        output
            .write_bytes(self.memoized_bytes.as_slice())
            .map_err(Into::into)
    }
...
pub struct ClosureArgument<
    R: Display + ?Sized + ToOwned<Owned = T> + 'static,
    T: Borrow<R> + Display + Clone + ?Sized + 'static,
>(Rc<RefCell<ClosureArgumentInner<R, T>>>);

impl<
        R: Display + ?Sized + ToOwned<Owned = T> + 'static,
        T: Borrow<R> + Display + Clone + ?Sized + 'static,
    > Clone for ClosureArgument<R, T>
{
    fn clone(&self) -> Self {
...
impl<
        R: Display + ?Sized + ToOwned<Owned = T> + 'static,
        T: Borrow<R> + Display + Clone + ?Sized + 'static,
    > Display for ClosureArgument<R, T>
{
...
impl<
        R: Display + ?Sized + ToOwned<Owned = T> + 'static,
        T: Borrow<R> + Display + Clone + ?Sized + 'static,
    > Generator for ClosureArgument<R, T>
{
    type Output = R;
